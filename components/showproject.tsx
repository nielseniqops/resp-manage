...
export default function ShowProject({projects, infScroll, setRef}:ProjectResponse){
    ...
    <div className="fixed w-full max-w-xl bg-white top-[5rem]">
        <div className="flex flex-row items-center justify-between h-10 pb-3 mt-4 text-xs font-bold text-gray-600 shadow-md ">
        ...
        { projects ?  projects.map( (project) =>{
          return(
            <Link href={`/project/${project?.id}`} key={project?.id}>
                <a className="flex flex-row items-center justify-between transition-all h-14 hover:bg-green-300">
                  ...
                  <div className="w-12 overflow-hidden text-sm text-center text-gray-600 whitespace-nowrap">{project?._count?.respondent}</div>
